...
static int ad_sym(ADec *d, const u32 *cum, u32 total) {
    ...
    /* +1/-1: naive floor((code-lo)*total/range) double-floors against the
     * encoder's floor(r*cum/total) bucket edges and can round down by one
     * exactly when code lands on a bucket's lower boundary, decoding the
     * previous symbol instead. This form is the correct inverse. */
    u32 val=(u32)((((u64)(d->code-d->lo)+1)*total-1)/range);
    int lo=0, hi=255;
...
int main(int argc, char **argv) {
    const char *inpath = (argc > 1) ? argv[1] : INPUT_PATH;
    size_t n; u8 *in=read_file(inpath,&n); if(!in) return 1;
    ...
    if (!ok && dlen==n) {
        int ndiff=0, first=-1;
        for (size_t i=0;i<n;i++) if (in[i]!=back[i]) { if(first<0) first=(int)i; ndiff++; }
        printf("  first mismatch at byte %d (in=%u back=%u), %d/%zu bytes differ\n",
               first, in[first], back[first], ndiff, n);
    }
    free(back);
